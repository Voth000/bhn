...
class Particle {
  ...
  draw() {
    ...
    for (let i = 0; i < this.textLines.length; i++) {
	  ...
	  ctx.fillText(this.textLines[i], this.x, this.y + i * (this.size + 2) - this.glowSize);
    }
  }
...
function createParticles(text) {
  const newParticles = [];
  for (let i = 0; i < 30; i++) {
    newParticles.push(new Particle(Math.random() * canvas.width, Math.random() * canvas.height, Math.random() * 10, text));
  }
...
document.getElementById('textForm').addEventListener('submit', event => {
  ...
  if (textCounter >= 4) {
    ...
    if (!alertShown) {
      ...
      alertBox.innerHTML = '<span> At this point, you reach a critical mental limit. </span> <span> Your brain shut all of your overthinking thoughts down.</span> <button class="alert-exit"> Go have fun </button> ';
